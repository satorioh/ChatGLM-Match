...
def init_chain_proxy(llm_proxy: LLM, vector_store, top_k=5):
    prompt_template = """你是一份有用的 PDF 文件。你的任务是提供信息并回答任何相关问题。你应该使用 PDF 文件的各个部分作为信息来源，尽可能简练准确地回答用户提出的任何问题。如果无法从中得到答案，请说 “根据已知信息无法回答该问题” 或 “没有提供足够的相关信息”，不允许在答案中添加编造成分。你需要在回答时引用相应的章节名称和页码，答案请使用中文。以下是有关该 PDF 文件的相关信息，这些信息将帮助你回答用户的问题。
